...
- `Parameters`：可调整 TP1/TP8、Prefill/Decode Batch、Hidden Size、
  Head、专家数、Cache、dtype 和峰值算力参数；架构约束由生成器代码校验，
  不在工作簿中单独展示。
...
本计算器只考虑推理，不包含反向传播、梯度、优化器或训练状态。通信量是每 Rank
的原始集合通信数据量；计算量是单次推理调用的单 Rank FLOPs；容量包含静态参数和
按 MaxContext 预分配的 KV/State。